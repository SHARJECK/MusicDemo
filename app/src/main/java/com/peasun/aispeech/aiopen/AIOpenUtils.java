...
public class AIOpenUtils {
    ...
    public static void registerMusicApp(Context context) {
        Intent localIntent = new Intent();
        localIntent.setAction(AIOpenConstant.AI_OPEN_ACTION_APP_REGISTER);
        Bundle data = new Bundle();
        data.putString("package_name", context.getPackageName());//music APP
        data.putString("api_package", context.getPackageName());//API APP
        data.putLong("category", AIOpenConstant.SEMANTIC_MUSIC);
    ...
    public static AIOpenReceiver registerMusicReceiver(Context context) {
